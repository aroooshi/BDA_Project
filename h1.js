d3.csv("data/loancleaned_2.csv", function(error, data) {
  ...
  // Scales
  var x_max = 20;
  var x_scale = d3.scale.linear().domain([500,35000]).range([0, hist_width]),
  ...
  rectangles.enter()
      .append('circle')
      .attr('cx', function(d){
        return x_scale(d['loan_amnt'])+margin.left;
        })
      .attr('cy', function(d){
        return y_scale(d['annual_inc_val'])-margin.top;
      })
      .attr('r', 1.5)
      ...
      .on('mouseover', function(d) {
                ...
                d3.select('.tooltip')
                    ...
                    .html('Loan Amount: <strong>'+d['loan_amnt']+'</strong><br />Annual Income: '+d['annual_inc_val']);
            })
